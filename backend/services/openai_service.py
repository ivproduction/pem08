...
class OpenAIService:
    ...
    async def analyze_text(self, text: str) -> CompetitorAnalysis:
        ...
        system_prompt = """Ты — эксперт по конкурентному анализу в нише ментального здоровья и онлайн-терапии. Проанализируй предоставленный текст конкурента и верни структурированный JSON-ответ.
...
Формат ответа (строго JSON):
{
    ...
    "mental_health_positioning": "Как компания позиционирует себя в нише ментального здоровья: целевая аудитория, подход (клинический/AI/коучинг), акцент на безопасности/доступности/эффективности"
}
...
- mental_health_positioning — одно-два предложения"""
    ...
    async def analyze_image(self, image_base64: str, mime_type: str = "image/jpeg") -> ImageAnalysis:
        ...
        system_prompt = """Ты — эксперт по визуальному маркетингу в нише ментального здоровья и онлайн-терапии. Проанализируй изображение сайта/лендинга конкурента и верни структурированный JSON-ответ.
...
Формат ответа (строго JSON):
{
    ...
    "trust_score": 8,
    "emotional_tone": "Эмоциональный тон визуала (например: спокойный и поддерживающий, тревожный, холодный, тёплый и т.д.)",
    "recommendations": ["рекомендация 1", "рекомендация 2", ...]
}

Важно:
- visual_style_score от 0 до 10: общее качество дизайна
- trust_score от 0 до 10: насколько дизайн внушает доверие и безопасность (критично для психологических сервисов)
- emotional_tone: одна фраза описывающая эмоциональный тон
- Каждый массив должен содержать 3-5 пунктов
- Пиши на русском языке
